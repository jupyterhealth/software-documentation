...
- **Python 3.11 or newer** - Required by the project. Check your version with `python --version` or `python3 --version`. If you don't have a suitable version, uv will download one for you, so this is optional
- **PostgreSQL** - Version 12 or newer. Download from [postgresql.org](https://www.postgresql.org/download/)
- **Git** - Download from [git-scm.com](https://git-scm.com/downloads)
- **uv** - Python package and project manager. Detailed installation instructions are provided in Step 1 below
...
The project's `pyproject.toml` requires Python 3.11 or newer. uv picks a suitable
interpreter on its own - and downloads one if none of yours qualify - so usually
you don't need to do anything. To pin a specific version:

```bash
# Use Python 3.12 specifically (same command on Mac, Linux and Windows)
uv sync --python 3.12
```
...
First, let's install uv, which will manage our Python dependencies.
...
**On Mac (with Homebrew), if you prefer:**
...
```{note}
uv is a self-contained binary, not a Python package, so the installers above
don't touch your system Python. If you've hit the "externally-managed-environment"
error (PEP 668) with other tools, it doesn't apply here - and you should never
work around that error with `--break-system-packages`, which can damage your
Python installation.
```

If you'd rather install it as a Python application, `pipx install uv` also works.

Close and reopen your terminal after installing so uv is in your PATH, then check
it's working:
...
This creates a `.venv` folder inside the project and installs the exact package versions recorded in `uv.lock`, matching what the developers use. The `--locked` flag makes uv fail rather than quietly re-resolve if the lockfile and `pyproject.toml` have drifted apart.

**What to expect:**

- This will download and install Django, PostgreSQL drivers, OAuth libraries, and 20+ other packages
- uv prints a short summary of what it resolved and installed
- It's fast - usually a few seconds, since uv downloads packages in parallel and caches them
- The final message should say something like "Installed 88 packages"

**If you see an error** about uv not being found, try closing and reopening your terminal so the installer's PATH change takes effect.

**If you see a Python version error**, pin an interpreter explicitly with `uv sync --python 3.12`, as mentioned in the prerequisites section.

**Important note about running commands:**

In the remaining steps, we'll run Python commands using `uv run python manage.py ...`. This automatically uses the correct Python environment without needing to activate a shell. It also re-checks the lockfile each time, so your environment stays up to date on its own.
...
uv doesn't spawn a sub-shell of its own - you activate the `.venv` it created in the usual way. Once activated, your command prompt will show `(jupyterhealth-exchange)` at the beginning, and you can run commands like `python manage.py migrate` without the `uv run` prefix.
...
This usually means you forgot to use `uv run` or to activate the environment. Make sure you're running commands with:
...
Once activated, you should see `(jupyterhealth-exchange)` at the start of your command prompt.
...
Or if you prefer activating the environment:
