...
| `JHE_CLIENT_SECRET`     | No       | The broker's confidential client secret at JHE (server-side only; copy immediately - hashed on save). Required for a confidential client; recommended.                                    |
| `MCP_RESOURCE_URL`      | Yes      | Public URL of this MCP server (e.g. `https://<your-mcp-host>`). Must be the server's own public URL.                                                                                      |
