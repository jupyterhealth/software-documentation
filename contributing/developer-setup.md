...
1. Set up your Python environment. This project uses Django **version 5.2** which requires python **3.10, 3.11, 3.12 or 3.13**.
   ```{note}
   The project uses [uv](https://docs.astral.sh/uv/): run `uv sync --frozen` to match the locked package versions.
   ```
...
1. Start the server (`.env` is loaded automatically via python-dotenv).
   ```shell
