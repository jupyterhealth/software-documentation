...
- Use the canonical `/FHIR/R5/` base for these. The lowercase `/fhir/r5/` alias kept for pre-#661 clients serves the same documents but is **not** CORS-enabled, so a browser app fetching it cross-origin will be blocked.
