...
1. Your client then needs to consume the the invitation code (`jhe.tcp.org_0wYuXvhoyRfko9yFYl9inpBiNkHLVBMy` in the example above) and exchange it for an access token that is specific to the patient. See [Auth](/jhe/auth) for instructions on how to do this.
...
1. Next, you need to present the Patient with the list of studies they have been invited to and the data scopes requested by the study. This is done is two steps below (see [Admin API](/jhe/admin-api) for examples):
...
1. After the Patient has agreed/consented to the studies and scopes presented in your UI, you need to record this by sending a `POST /api/v1/patients/40001/consents` (see [Admin API](/jhe/admin-api) for examples). Equivalent `PATCH` and `DELETE` requests can be used to update or revoke consents from your UI.
